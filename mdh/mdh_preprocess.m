...
%    Hamid Behjat
% 2017-2018.
% 2018-2020 - Modified by: Anjali Tarun
...
% Files.
f.ref = fullfile(f.diffusion,'nodif_brain_mask.nii.gz');
f.ref_125 = fullfile(f.diffusion,'nodif_brain_mask.nii');
if ~exist(f.ref_125,'file')
    gunzip(f.ref)
end
...
% end
% 
% if ~exist(f.ribbon_125_conn,'file')
%     hb_make_single_connected(f.ribbon_125,6,'outputFileName',...
%         f.ribbon_125_conn,'alwaysWriteConnected',1,'sS',0);
% end
% 
% if ~exist(f.gmwm_125 ,'file')
%    V = spm_read_vols(spm_vol(f.ribbon_125_conn))+...
%        spm_read_vols(spm_vol(f.white_125)); 
%    d = spm_vol(f.white_125);
%    d.fname = f.gmwm_125;
%    spm_write_vol(d,V);
% end
% 
...
% 
% % Surface files.
% surfDir = hb_get_dir('hcp_t1w_surf','baseDir',hcp_root,'subjectID',subjID);
% f.pialFiles = {...
%     fullfile(surfDir,'lh.pial.surf.gii'),...
%     fullfile(surfDir,'rh.pial.surf.gii')};
